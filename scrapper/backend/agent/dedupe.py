...
def dedupe_all_for_job(job_id: str) -> dict:
    ...
    into it, and deletes the extras.
    """
    print(f"🔁 [Dedupe] job_id={job_id}")
    rows = db.list_contractors_for_job(job_id)
    ...
    for dup_id, surv_id in merge_into.items():
        ...
        acc = survivor_updates.setdefault(surv_id, {
            "place_ids": set(surv.get("place_ids") or []),
            "sources": set(surv.get("sources") or []),
        })
        acc["place_ids"].update(dup.get("place_ids") or [])
        acc["sources"].update(dup.get("sources") or [])

    for surv_id, acc in survivor_updates.items():
        db.update_contractor(surv_id, {
            "place_ids": sorted(acc["place_ids"]),
            "sources": sorted(acc["sources"]),
        })

    if merge_into:
        db.delete_contractors_by_ids(list(merge_into.keys()))
